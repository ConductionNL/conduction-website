...
Dit deel is alleen relevant in één specifiek geval: **een oudere Conduction-app moet alsnog onder de huidige OpenSpec-conventie gebracht worden**. Een klein aantal van onze apps is gebouwd voordat `openspec/specs/` en de `@spec`-tag-conventie bestonden — ze hebben werkende code, maar geen specs en geen annotaties die vanuit code terug wijzen naar requirements. *Als* zo'n app ooit in lijn met de conventie gebracht moet worden, is dit de playbook ervoor. We lopen de [retrofit-playbook](https://github.com/ConductionNL/.github/blob/main/docs/claude/retrofit.md) van begin tot eind door.

{/* truncate */}

> **Overleg eerst met je team lead voordat je begint.** Een retrofit is echt werk — geen losse opdracht die je tussendoor doet. De volledige pass op een middelgrote app kost al snel uren wandkloktijd en een flinke hoeveelheid Claude-tokens (vaak de kosten van een gewone feature-change, soms meer — zeker als Bucket 2 groot uitvalt en `/opsx-reverse-spec` op veel clusters moet draaien). De retrofit-playbook noemt expliciet een [roll-out-volgorde over onze apps](https://github.com/ConductionNL/.github/blob/main/docs/claude/retrofit.md#roll-out-order) precies om die reden: niet elke app is nu aan de beurt, en niet elke app heeft het nu nodig. Draai geen retrofit op eigen initiatief — stem eerst met je team lead of product owner af of deze app de juiste kandidaat is, welk budget er is, en welke apps hogere prioriteit hebben. Daarna start je bij Stap 1.
...
Vrijwel elke Conduction-app is gebouwd **voordat** de spec ↔ code annotatie-conventie uit [ADR-003](https://github.com/ConductionNL/hydra/blob/main/openspec/architecture/adr-003-backend.md) bestond. Die conventie zegt: het hoofd-docblock van elk PHP-bestand én het docblock van elke public method dragen één of meer `@spec openspec/changes/<change-name>/tasks.md#task-N`-tags die wijzen naar de taak(en) die ze implementeren.
...
| --- | --- | --- |
| [`/opsx-coverage-scan`](https://github.com/ConductionNL/.github/blob/main/docs/claude/commands-openspec.md#opsx-coverage-scan) | **Alleen auditen.** Indeling van elke code-unit in een van zes categorieën. | `openspec/coverage-report.md` + `.json`-sidecar |
| [`/opsx-annotate`](https://github.com/ConductionNL/.github/blob/main/docs/claude/commands-openspec.md#opsx-annotate) | Voegt `@spec`-tags toe aan elke Bucket 1-methode via een **ghost change**. | Annotatie-only PR |
| [`/opsx-reverse-spec`](https://github.com/ConductionNL/.github/blob/main/docs/claude/commands-openspec.md#opsx-reverse-spec) | Draft REQs voor een Bucket 2-cluster via een ghost change, en annoteert. | Eén spec-PR per cluster |
...
- **PHPCS weigert de tag-volgorde?** Stop. **Fix de PHPCS-config, herorder nooit de tags.** Het ADR-003 + [hydra-gate-spdx](https://github.com/ConductionNL/hydra/blob/main/.claude/skills/hydra-gate-spdx/SKILL.md)-formaat ligt vast.
...
<NextSteps title="Volgende stap" lede="Met de retrofit-playbook in handen liggen er twee logische vervolgen:">
  ...
  <NextStep
    ...
    href="https://github.com/ConductionNL/.github/blob/main/docs/claude/retrofit.md"
