...
Voeg de `knowledge_article`-slug toe aan de kleine lijst `SettingsService::SCHEMA_SLUGS` in [lib/Service/SettingsService.php](https://codeberg.org/Conduction/deskdesk/src/branch/main/lib/Service/SettingsService.php), zodat `/api/settings` het numerieke id naast de andere schema's blootlegt. Hetzelfde in `src/store/store.js`:
...
(Zie de [reference repo](https://codeberg.org/Conduction/deskdesk/src/branch/main/src/App.vue) voor het volledige bestand. App.vue voorziet ook in `objectSidebarState`, zodat het externe-sidebar-kanaal van CnDetailPage werkt.)
...
Voor de **Conduction app store** (`apps.conduction.nl`) duwt dezelfde release-pipeline naar dat endpoint als de secret is geconfigureerd. Zie [de .github/workflows/release.yml](https://codeberg.org/Conduction/nextcloud-app-template/src/branch/main/.github/workflows/release.yml) als referentie.
...
<NextSteps lede="Deel 5 en Deel 6 staan naast elkaar — ze breiden de uitgeleverde app uit in twee onafhankelijke richtingen. Pak ze in willekeurige volgorde, of kies degene die je volgende app het eerst nodig heeft. Deel 5 verdiept het manifest-vlak; Deel 6 verbreedt naar andere systemen. Geen van beide verwijst naar de ander voor inhoud.">
  ...
  <NextStep
    ...
    description="Zie hoe Procest, OpenCatalogi, LaunchPad en de rest dezelfde patronen samenstellen. DeskDesk hoort nu bij die rij."
  />
</NextSteps>
