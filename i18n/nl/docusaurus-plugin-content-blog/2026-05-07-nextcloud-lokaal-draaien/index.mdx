...
> **Dagelijkse developer-setup?** Werk je door de [Conduction Workstation Setup-leerlijn](/nl/academy/workstation-tutorial-1-wat-installeer-je) heen, dan pakt [Deel 5](/nl/academy/workstation-tutorial-5-nextcloud-lokaal) op waar deze tutorial eindigt — met de Conduction-stack-compose-flow (de `docker-compose.yml` in de [`openregister`](https://codeberg.org/Conduction/openregister)-repo met de `--profile`-flags en `clean-env.sh`-reset) die de standalone compose hieronder niet behandelt.
