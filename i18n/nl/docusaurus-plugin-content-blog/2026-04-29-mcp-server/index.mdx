...
<Prerequisites title="Wat je nodig hebt">
  <PrerequisiteItem>Een draaiende lokale Nextcloud met de OpenRegister-app geïnstalleerd en minstens één register + schema gedefinieerd. De snelste weg daarheen is <a href="/nl/academy/workstation-tutorial-5-nextcloud-lokaal">Workstation Deel 5 — Nextcloud lokaal draaien</a>.</PrerequisiteItem>
...
Je krijgt een JSON-document terug met de capability-gebieden die de server adverteert (registers, schemas, objects, audit, …). Krijg je in plaats daarvan een HTML-loginpagina, dan draait je lokale Nextcloud wel maar is de OpenRegister-app niet geactiveerd — fix dat met:
...
| `objects` | `list`, `get`, `create`, `update`, `delete` | Objecten gevalideerd tegen een schema, opgeslagen in een register. Alle acties vereisen zowel `registerId` als `schemaId`. |
...
| `openregister://objects/{registerId}/{schemaId}` | Alle objecten in één register + schema-combinatie. |
| `openregister://registers/{id}` | Eén register op ID. |
| `openregister://schemas/{id}` | Eén schema op ID. |
| `openregister://objects/{registerId}/{schemaId}/{uuid}` | Eén object op samengestelde sleutel. |
...
Voor een uitgebreide rondleiding wanneer welk van de twee past, zie [Claude Skills tutorial — Deel 1: Wat zijn Claude Skills?](/nl/academy/claude-skills-tutorial-1-wat-zijn-claude-skills).
...
<NextSteps title="Wat nu" lede="Je hebt een werkende datalaag-MCP-server. Twee logische vervolgstappen:">
  ...
  />
  <NextStep
    title="Het OpenRegister-project op Codeberg"
