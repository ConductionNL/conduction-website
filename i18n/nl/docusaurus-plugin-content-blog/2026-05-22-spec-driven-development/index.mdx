...
tags: [App-ontwikkeling, Spec-gedreven, OpenSpec, Hydra, ADR, AI, n8n, Windmill, OpenBuild]
apps: [openregister]
durationMinutes: 25
---
...
<Prerequisites title="Waar je moet staan">
  ...
  <PrerequisiteItem>De OpenSpec-skills geïnstalleerd — de <code>{'opsx-*'}</code>-familie zit in de <a href="https://codeberg.org/Conduction/hydra">ConductionNL/hydra</a>-repository. Clone die (of kopieer de <code>.claude/skills/</code> naar je project) zodat de <code>{'/opsx-*'}</code>-commando's beschikbaar zijn.</PrerequisiteItem>
</Prerequisites>
...
Dit is wat **OpenBuild** — Conductions visuele app-builder — mogelijk maakt. Een citizen developer sleept schemas op een canvas, wijst workflow-hooks naar n8n-workflows aan, en levert een werkende app op **zonder ook maar één regel code te schrijven**. De "code alleen wanneer het moet"-belofte wordt letterlijk: er is geen code in de app om te schrijven, omdat het schema-register + de n8n-workflow + het manifest al alles afdekken wat een LLM (of een mens, of OpenBuild) nodig heeft om een complete app te produceren.

→ [OpenBuild](https://conduction.nl/apps/openbuild) — de visuele app-builder. Hetzelfde OpenSpec-contract, hetzelfde schema-register, dezelfde workflow-engine-adapters; alleen geen JSON-editor.
