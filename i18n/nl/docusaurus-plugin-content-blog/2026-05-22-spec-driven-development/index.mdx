...
In het geval van Conduction zijn de organisatie-niveau ADR's de fleet-brede set die elke Conduction-app erft (datalaag, frontend, beveiliging, i18n, testen, licentie, de app-manifest-conventie, schema-declaratieve business logic, spec-sizing). Voor een concreet, browsebaar voorbeeld van **applicatie-niveau** ADR's vormen die van Integriq goede leesvoer — 16 stuks, van [`adr-001-domain-pinia-stores-app-local`](https://codeberg.org/Conduction/openconnector/src/branch/main/openspec/architecture) tot het ontwerp van de encryption-service, elk een echte lokale beslissing die de rest van de fleet niet bindt.
