    ...
    <div className="cn-filter-layout">
      ...
      <div>
        <div style={{display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 24, paddingBottom: 16, borderBottom: '1px solid var(--c-cobalt-100)', fontSize: 14, color: 'var(--conduction-color-text-body)'}}>
          <div>
            <strong style={{color: 'var(--conduction-color-text-strong)'}}>{filtered.length}</strong> van {sidecars.length} sidecars
            {totalSelected > 0 && <span style={{color: 'var(--conduction-color-text-muted)'}}> · {sidecars.length - filtered.length} verborgen door filters</span>}
          </div>
        </div>
        ...
        {filtered.length === 0 && (
          <p style={{textAlign: 'center', color: 'var(--conduction-color-text-default)', padding: 'var(--space-8) 0'}}>Geen sidecars passen bij dit filter.</p>
        )}
      </div>
    </div>
  );
