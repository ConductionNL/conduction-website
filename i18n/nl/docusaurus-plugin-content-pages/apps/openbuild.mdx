...
<DetailHero
  ...
  locales="NL · EN"
  title="OpenBuild"
  tagline={<>Een app-bouwer voor citizen developers op <span className="next-blue">Nextcloud</span>. Stel je eigen app samen uit de Technical Core — OpenRegister-schema's, OpenConnector-API's, Procest-workflows en DocuDesk-documenten. Geen PHP, geen scaffolding.</>}
  primaryCta={{label: 'Installeer uit de app store', href: '/install'}}
  secondaryCta={{label: 'Lees de docs', href: 'https://openbuild.conduction.nl'}}
...
OpenBuild is een open-source app-bouwer voor citizen developers in de Nextcloud-workspace. Gebruikers stellen een werkende app samen uit getypeerde OpenRegister-registers, OpenConnector-koppelvlakken, Procest-workflows en DocuDesk-sjablonen. Daarna leg je in een browser-designer de lijst-, detail- en formulier-pagina's neer. Configuration-over-code manifest-overrides overleven upgrades, versie-snapshots draaien foute edits terug, ZIP- of GitHub-exports verhuizen apps tussen instances, en RBAC houdt per-record toegang afgedwongen via OpenRegister. Uitgegeven onder EUPL-1.2, onderhouden door Conduction sinds 2019.
