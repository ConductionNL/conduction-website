...
<DetailHero
  ...
  status={{label: 'In development', color: 'var(--conduction-color-text-muted)'}}
  version="v0.x"
  locales="NL · EN"
  title="Versioniq"
...
<Section spacing="default">
  <SectionHead
    ...
    align="stack"
    lede="Nextcloud's built-in updater jumps to the latest. Versioniq (formerly App Versions) adds the missing knob: pick any release, from any configured source, and pin or roll back without leaving the admin UI. The change is recorded, so when something breaks two days later you know who downgraded what."
  />
  <FeatureList>
    <FeatureItem
...
<CtaBanner
  ...
  lede={<>Versioniq is the difference between 'roll forward and hope' and 'roll back if it bites'. Open-source, admin-only, no extra service to run.</>}
