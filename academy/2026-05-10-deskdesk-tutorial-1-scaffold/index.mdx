...
- **App source:** [`ConductionNL/deskdesk`](https://codeberg.org/Conduction/deskdesk) — the actual Nextcloud app you're forking from the template.
- **Tutorial source:** [`ConductionNL/conduction-website`](https://codeberg.org/Conduction/conduction-website) — the post you're reading right now.
...
Every Conduction app — DeskDesk, OpenRegister, OpenCatalogi, Procest, LaunchPad, the dozen others — looks the same way on first sight. Same five structural pieces, same place, same behaviour. That recognisability is what `@conduction/nextcloud-vue` enforces: a user who learnt one app navigates the next one without docs.
