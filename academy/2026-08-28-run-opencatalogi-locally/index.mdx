---
slug: run-opencatalogi-locally
title: Run OpenCatalogi locally
contentType: tutorial
heroIcon: book-open
authors: [conduction]
date: 2026-08-28
summary: One compose file and about ten minutes. You end with OpenCatalogi running on your own machine on real Nextcloud, backed by OpenRegister, with a way to check that it actually worked rather than merely loaded.
tags: [Demo, Docker, Local, OpenCatalogi, OpenRegister]
apps: [opencatalogi, openregister, nldesign, openconnector, portaliq]
durationMinutes: 10
---
...
<Outcomes>
  <Outcome>OpenCatalogi running locally on real Nextcloud, with OpenRegister behind it.</Outcome>
...
```bash
curl -fsSLO https://raw.githubusercontent.com/ConductionNL/opencatalogi/development/opencatalogi-compose.yaml
```
...
| What | Where |
| --- | --- |
| **OpenCatalogi** | [http://localhost:8600/apps/opencatalogi/](http://localhost:8600/apps/opencatalogi/) |
...
| App | Why |
| --- | --- |
| `openregister` | **Required.** OpenCatalogi declares its registers and schemas against OpenRegister. |
...
| `opencatalogi` | The app this tutorial is about. |

That OpenRegister dependency is **not declared** in `appinfo/info.xml`. No app in the fleet declares an `<app>` dependency, so nothing stops the App Store installing OpenCatalogi on its own — it would then load, find no register to attach to, and show you an empty app rather than an error. The compose file encodes the dependency the manifest does not.
...
curl -s -u admin:admin "http://localhost:8600/apps/openregister/api/registers" | head -c 300
```

## What to look at first

OpenCatalogi ships with more than fits in one sitting. These are the parts worth opening first:

- **WOO disclosure workflow**
- **DCAT-AP harvest**
- **Federated directory**
...
Nextcloud is waiting for an upgrade. Run `docker compose -f opencatalogi-compose.yaml exec -u www-data nextcloud php occ upgrade`.
</TroubleshootingItem>

<TroubleshootingItem symptom="The port is already in use">
Set another one: `DEMO_PORT=9000 docker compose -f opencatalogi-compose.yaml up -d`. The port also has to appear in the trusted-domain list, which the compose file handles for you from the same variable.
</TroubleshootingItem>
...
<NextSteps title="Next step" lede="A running app is the entry point, not the destination.">
  ...
  <NextStep href="/apps/opencatalogi" title="What OpenCatalogi does" cta="Read more">
