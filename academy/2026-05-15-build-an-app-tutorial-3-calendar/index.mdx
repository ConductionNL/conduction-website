...
## Why not Integriq or a custom listener?

When you first hear "sync PetStore order ship-dates to Nextcloud Calendar" your instinct is probably one of:

1. **Write an Integriq flow** that listens to `OrderCreatedEvent` and POSTs to the CalDAV endpoint
...
The same pattern is followed by [the spec](https://github.com/ConductionNL/openregister/blob/development/openspec/specs/calendar-provider/spec.md).
...
<NextSteps>
  <NextStep
    ...
    description="Author the category care-guides in xWiki, wire Integriq to surface them per pet in CnObjectSidebar, package the app, publish to apps.conduction.nl."
  />
  <NextStep
