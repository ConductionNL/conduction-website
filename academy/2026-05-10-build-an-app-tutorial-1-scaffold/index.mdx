...
# --legacy-peer-deps skips the webpack-vue-config peer chain. Install them explicitly:
npm install --legacy-peer-deps sass sass-loader linkifyjs node-polyfill-webpack-plugin terser-webpack-plugin vue-loader
npm run build
```

:::tip Why the second `npm install`
`--legacy-peer-deps` tells npm not to resolve peer dependencies. The template's webpack config (`@nextcloud/webpack-vue-config`) needs `sass`, `sass-loader`, `node-polyfill-webpack-plugin`, `terser-webpack-plugin`, and `vue-loader` as peers. Without the second install the build fails with `Cannot find module 'node-polyfill-webpack-plugin'`. Installing them all in one command avoids the npm `--no-save` reconciliation trap (each install prunes previously unsaved packages).
:::
