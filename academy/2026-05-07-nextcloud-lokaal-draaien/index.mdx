...
> **Day-to-day developer setup?** If you're working through the [Conduction Workstation Setup tutorial chain](/academy/workstation-tutorial-1-wat-installeer-je), [Part 5](/academy/workstation-tutorial-5-nextcloud-lokaal) picks up where this tutorial leaves off — covering the Conduction-stack compose flow (the `docker-compose.yml` inside the [`openregister`](https://codeberg.org/Conduction/openregister) repo with its `--profile` flags and `clean-env.sh` reset) that the standalone compose below doesn't have.
