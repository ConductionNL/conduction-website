...
2. **"Skip if nothing new was learned"** is cruciaal. Zonder die regel produceert Claude *altíjd* iets — pseudo-inzichten zoals "de skill werd succesvol uitgevoerd". Dat is precies het soort ruis dat het [`update-skill-overview.sh`](https://github.com/ConductionNL/hydra/blob/main/.claude/skills/update-skill-overview.sh) script niet kan filteren maar wel je context-budget opslokt.
...
### Orkestratie-patronen die we in Hydra gebruiken
...
Daarvoor hebben we in Hydra twee bestanden:

- **[`.claude/skills/skill-level-overview.html`](https://github.com/ConductionNL/hydra/blob/main/.claude/skills/skill-level-overview.html)** — een lokaal, statisch HTML-dashboard. Eén tabel met per skill: huidige maturity (groene dots), target maturity (badge), regelaantal van `SKILL.md` (gekleurd: groen ≤200, geel rond 450, rood >500), en oranje rings voor "structureel aanwezig maar nog niet mature". Sorteerbaar per kolom.

  > **Hoe bekijk je hem als webpagina?** GitHub toont HTML-bestanden altijd als broncode — niet als gerenderde pagina. Open hem daarom lokaal nadat je de repo gecloned hebt: `xdg-open hydra/.claude/skills/skill-level-overview.html` (Linux), `open …` (macOS) of dubbelklik in je file-explorer. Niet gecloned? Eén bestand pakken met `gh api repos/ConductionNL/hydra/contents/.claude/skills/skill-level-overview.html --jq '.content' | base64 -d > /tmp/skill-overview.html && xdg-open /tmp/skill-overview.html`. Voor publieke repos werkt ook `https://htmlpreview.github.io/?<github-blob-url>` — maar Hydra is privé, dus daar krijg je een 404.
- **[`.claude/skills/update-skill-overview.sh`](https://github.com/ConductionNL/hydra/blob/main/.claude/skills/update-skill-overview.sh)** — een bash-script dat over alle skill-folders heen scant, structurele markers detecteert (frontmatter aanwezig, guardrails, evals met genoeg trigger-tests, dated learnings, agent-spawns), en de HTML up-to-date schrijft. Eén `./update-skill-overview.sh` en je dashboard klopt weer.

### Wat het script automatisch doet

Het script (zie de [bron op main](https://github.com/ConductionNL/hydra/blob/main/.claude/skills/update-skill-overview.sh)) detecteert:
...
Wil je het script en dashboard buiten Hydra gebruiken? Twee stappen:

1. Kopieer `skill-level-overview.html` en `update-skill-overview.sh` naar `<jouw-repo>/.claude/skills/` (of `~/.claude/skills/` voor je globale library).
2. Draai `bash .claude/skills/update-skill-overview.sh`. De eerste run scant alles en seedt nieuwe rijen voor skills die nog niet in de HTML stonden.

Een natuurlijke plek om hem te draaien: vóór een release-tag, in een pre-commit hook op de skills-folder, of als wekelijkse cron. Zo zie je drift (een skill die naar 520 regels groeit en dus L2 verliest) meteen.
