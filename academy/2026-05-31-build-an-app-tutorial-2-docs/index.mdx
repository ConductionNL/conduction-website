---
slug: build-an-app-tutorial-2-docs
title: "Build your first Conduction app — Part 2: A documentation site with screen mocks and captured walkthroughs"
contentType: tutorial
authors: [conduction]
date: 2026-05-31
summary: Stand up the docs site for the PetStore app you built in Part 1. Hosted from your own Codeberg or GitHub repo via the central documentation workflow, illustrated with token-built screen mocks, and walked through with Playwright-captured screenshots.
...
A 45-minute tutorial that gives your app a documentation site. Hosted from your own Codeberg or GitHub repo via the central documentation workflow, illustrated with abstract screen mocks that compose from design tokens, and brought to life with real screenshots that Playwright captures from your running app. By the end, every push to `development` rebuilds and republishes.

{/* truncate */}

:::tip Recommended first — Part 1
Part 2 picks up where Part 1 left off. You already have a Codeberg or GitHub repo, the PetStore schemas in OpenRegister, and a working OpenConnector flow. If you don't yet, take [Part 1: Schemas, registers, and a working flow](/academy/build-an-app) first (30 minutes).
:::

:::tip Also useful first — the Claude Skills tutorial series
Three of the steps below run Conduction skills (`/journeydoc-init`, `/journeydoc-instrument`, `/journeydoc-add-story`). Skills are short, declarative Claude Code workflows. If you've never run one, take [Part 1 of the Claude Skills series](/academy/claude-skills-tutorial-1-wat-zijn-claude-skills) first (10 minutes).
:::
...
<Prerequisites title="What you need">
  <PrerequisiteItem>Part 1 completed: <a href="/academy/build-an-app">your PetStore app</a> with two schemas (<code>pet</code>, <code>customer</code>), a register, and a working OpenConnector flow.</PrerequisiteItem>
...
3. Asks for your user-track stories, one per line. For PetStore: *Open the app for the first time*, *Add a pet*, *Edit a pet's status*, *Search the catalogue*.
4. Asks for admin-track stories, or skip if PetStore has no admin UI.
...
- `docs/tutorials/user/01-first-launch.md`, `02-add-pet.md`, `03-edit-pet-status.md`, `04-search-catalogue.md` as skeleton pages, one per story
...
# 01-first-launch.md  02-add-pet.md  03-edit-pet-status.md  04-search-catalogue.md

cat docs/static/CNAME
# petstore.conduction.nl
```

Merge the PR before continuing. Part 2 builds on the scaffold.
...
PetStore's launch dashboard ships three KPI widgets and an activity feed:
...
# 24:    <input data-testid="pet-form.species" v-model="form.species" />
...
```bash
/journeydoc-add-story petstore user "Add a pet to the store"
```
...
| Steps, paste a numbered list of UI clicks. | 1. Open the PetStore app. 2. Click "Add pet". 3. Fill name + species. 4. Click Save. 5. See the new row in the list. |
...
test('U02 add-pet — Add a pet to the store', async ({ page }) => {
  ...
  await page.getByTestId('pet-form.species').fill('Cat');
...
## What you ship at the end of Part 2
...
Part 3 of this series (forthcoming) covers publishing your app to the Nextcloud app store: signing the release, the `appstoreapp:create-app` flow, and shipping under your own brand. Until then, the [Nextcloud app store docs](https://nextcloudappstore.readthedocs.io/) describe the upload protocol.

<NextSteps>
  <NextStep
    title="Part 1 — Schemas, registers, and a working flow"
    href="/academy/build-an-app"
    summary="Back to the PetStore build, in case you need to revisit the schema-and-flow setup."
  />
  <NextStep
  ...
  />
  <NextStep
    title="DeskDesk tutorial series — Part 4: Knowledge and ship"
    href="/academy/deskdesk-tutorial-4-knowledge-and-ship"
    summary="An applied take on shipping an app's knowledge surface end-to-end, with worked DeskDesk examples."
  />
</NextSteps>
