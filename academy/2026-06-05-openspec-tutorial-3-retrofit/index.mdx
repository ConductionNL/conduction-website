...
Almost every Conduction app was built **before** the spec ↔ code annotation convention from [ADR-003](https://github.com/ConductionNL/hydra/blob/main/openspec/architecture/adr-003-backend.md). That convention says: every PHP file's main docblock and every public method's docblock carries one or more `@spec openspec/changes/<change-name>/tasks.md#task-N` tags pointing at the task(s) it implements.
...
| --- | --- | --- |
| [`/opsx-coverage-scan`](https://github.com/ConductionNL/.github/blob/main/docs/claude/commands-openspec.md#opsx-coverage-scan) | **Audit only.** Buckets every code unit into one of six categories. | `openspec/coverage-report.md` + `.json` sidecar |
| [`/opsx-annotate`](https://github.com/ConductionNL/.github/blob/main/docs/claude/commands-openspec.md#opsx-annotate) | Adds `@spec` tags to every Bucket 1 method via a **ghost change**. | Annotation-only PR |
| [`/opsx-reverse-spec`](https://github.com/ConductionNL/.github/blob/main/docs/claude/commands-openspec.md#opsx-reverse-spec) | Drafts REQs for a Bucket 2 cluster via a ghost change, then annotates. | One spec PR per cluster |
...
- **PHPCS rejecting the tag order?** Stop. **Fix the PHPCS config, never reorder the tags.** The ADR-003 + [hydra-gate-spdx](https://github.com/ConductionNL/hydra/blob/main/.claude/skills/hydra-gate-spdx/SKILL.md) format is fixed.
