...
<Prerequisites title="What you'll need">
  ...
  <PrerequisiteItem>A clean working tree on the branch that carries the specs (usually <code>development</code>, but a few apps like <code>decidesk</code> keep specs on <code>beta</code> — check first). The retrofit skills refuse a dirty tree.</PrerequisiteItem>
...
3. Archives the ghost change immediately (it never needs review on its own).
...
<Troubleshooting title="Troubleshooting">
  <TroubleshootingItem symptom="/opsx-annotate refuses to run — 'dirty working tree'">
    The retrofit skills refuse a dirty tree on purpose: an annotation pass touches hundreds of files, and you really don't want to mix that with unrelated edits. Stash or commit your other changes first.
  </TroubleshootingItem>
