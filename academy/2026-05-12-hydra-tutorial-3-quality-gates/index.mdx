...
On top of the generic tools, Hydra has **its own set of `hydra-gate-*` skills** for things that are Conduction-specific. They live in `hydra/.claude/skills/`, but the single source of truth is the shell script [`scripts/run-hydra-gates.sh`](https://github.com/ConductionNL/hydra/blob/main/scripts/run-hydra-gates.sh). One dispatcher skill — `hydra-gates` — runs all of them one after another and produces a single pass/fail summary. The last line of a clean run reads:
