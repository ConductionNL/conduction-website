...
<Prerequisites title="What you need">
  <PrerequisiteItem>A running local Nextcloud with the OpenRegister app installed and at least one register + schema defined. The fastest way there is <a href="/academy/workstation-tutorial-5-nextcloud-lokaal">Workstation Part 5 — Run Nextcloud locally</a>.</PrerequisiteItem>
...
You should get a JSON document listing the capability areas the server advertises (registers, schemas, objects, audit, …). If you get an HTML login page back instead, your local Nextcloud is up but the OpenRegister app isn't enabled — fix that with:
...
| `objects` | `list`, `get`, `create`, `update`, `delete` | Objects validated against a schema, stored in a register. All actions require both `registerId` and `schemaId`. |
...
| `openregister://objects/{registerId}/{schemaId}` | All objects in one register + schema combination. |
| `openregister://registers/{id}` | One register by ID. |
| `openregister://schemas/{id}` | One schema by ID. |
| `openregister://objects/{registerId}/{schemaId}/{uuid}` | One object by composite key. |
...
For an in-depth tour of when each one fits, see [Claude Skills tutorial — Part 1: What are Claude Skills?](/academy/claude-skills-tutorial-1-wat-zijn-claude-skills).
...
<NextSteps title="What's next" lede="You've got a working data-layer MCP server. Two natural follow-ups:">
  ...
  />
  <NextStep
    title="The OpenRegister project on Codeberg"
