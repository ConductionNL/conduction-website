---
slug: run-hermiq-locally
title: Run Hermiq locally
contentType: tutorial
heroIcon: bot
authors: [conduction]
date: 2026-08-28
summary: One compose file and about ten minutes. You end with Hermiq running on your own machine on real Nextcloud, backed by OpenRegister, with a way to check that it actually worked rather than merely loaded.
tags: [Demo, Docker, Local, Hermiq, OpenRegister]
apps: [hermiq, openregister, nldesign, openconnector]
durationMinutes: 10
---
...
Hermiq is a Nextcloud app built on OpenRegister. Reading about what it does is not the same as clicking through it, and standing up a Nextcloud by hand to find out is more work than most people will do on a first look.
...
<Outcomes>
  <Outcome>Hermiq running locally on real Nextcloud, with OpenRegister behind it.</Outcome>
...
| App | Why |
| --- | --- |
| `openregister` | **Required.** Hermiq declares its registers and schemas against OpenRegister. |
...
| `hermiq` | The app this tutorial is about. |

That OpenRegister dependency is **not declared** in `appinfo/info.xml`. No app in the fleet declares an `<app>` dependency, so nothing stops the App Store installing Hermiq on its own — it would then load, find no register to attach to, and show you an empty app rather than an error. The compose file encodes the dependency the manifest does not.
...
Hermiq ships with more than fits in one sitting. These are the parts worth opening first:
...
Nextcloud is waiting for an upgrade. Run `docker compose -f hermiq-compose.yaml exec -u www-data nextcloud php occ upgrade`.
</TroubleshootingItem>

<TroubleshootingItem symptom="The port is already in use">
Set another one: `DEMO_PORT=9000 docker compose -f hermiq-compose.yaml up -d`. The port also has to appear in the trusted-domain list, which the compose file handles for you from the same variable.
</TroubleshootingItem>
...
<NextSteps title="Next step" lede="A running app is the entry point, not the destination.">
  ...
  <NextStep href="/apps/hermiq" title="What Hermiq does" cta="Read more">
