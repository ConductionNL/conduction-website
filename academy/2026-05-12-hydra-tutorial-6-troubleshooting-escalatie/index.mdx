...
tags: [Hydra, Troubleshooting, Recovery, Intern, Tutorial series]
apps: []
durationMinutes: 15
...
<HexCard title="spdx-headers + forbidden-patterns loop (2026-04-19)">
  Op decidesk #44/#71/#72 hetzelfde patroon: builder mist EUPL-1.2 header op nieuwe PHP-files, reviewer claimt 0 fixes nodig, recheck slaat rood. Forbidden-patterns bleek deels fout-positief (grep zonder word-boundary). Fix: handmatig SPDX toegevoegd + grep aangescherpt.
</HexCard>

<HexCard title="decidesk #44/#45 — gate-7 stub-method exploit (2026-04-23)">
  Vijf onderscheiden pipeline-bugs in één retrospectief. Builder leverde een methode op met <code>return null;</code> + TODO; PHPCS slikte het; geen test; productie-bug. Resulteerde in <code>hydra-gate-stub-scan</code> + ADR-021 (bounded-fix scope op shape, niet op lijntelling). Volledige lessen in <a href="https://github.com/ConductionNL/hydra/blob/main/docs/retrospectives/decidesk-44-45-phase-g.md">decidesk-44-45-phase-g.md</a>.
</HexCard>
...
- Een kleine semantische beslissing die de Builder niet kan maken zonder context buiten de spec (bijv. "moet dit veld optional of required zijn?").
...
1. **In welke volgorde probeer je de interventies in de ladder, en waarom?**  
   *(Hint: van goedkoop naar duur; behoud zoveel mogelijk reeds gedaan werk.)*

2. **Wat is het verschil tussen "Reviewer-labels resetten" en <code>retry:queued</code>?**  
   *(Hint: één pakt de builder-output ongemoeid, de andere dispatch de builder opnieuw met feedback.)*

3. **Wanneer is het zinvoller om met de hand de fix te maken in plaats van opnieuw <code>retry:queued</code>?**  
   *(Hint: denk aan herhaalde failures op dezelfde mechanische gate.)*

4. **Wat doe je als je 3+ identieke <code>needs-input</code>-comments ziet binnen korte tijd?**  
   *(Hint: dat is geen retry-vraag, dat is een infra-bug-indicator.)*
