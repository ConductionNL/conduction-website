...
| **Reviewer** | Juan Claude | `hydra-gates` + 8 `hydra-gate-*` skills + vendor `code-review` | Mandatory hydra-gates check + diepere code-review via de Anthropic community-skill. |
| **Security** | Clyde Barcode | `hydra-gates` + 8 `hydra-gate-*` skills + vendor `trailofbits` + vendor `owasp` | Mandatory hydra-gates + SAST (Semgrep) + OWASP-top-10-checklists. |
...
Die drie zitten dus **wél** in de fabriek (containers). We laden ze bij Juan en Clyde voor extra dekking boven op de eigen `hydra-gate-*`. Onderhoud ligt bij externe partijen — we updaten ze door de bron te volgen, niet door zelf te editen.
...
- **Reviewer** — `hydra-gates` + 8 individuele `hydra-gate-*` + vendor `code-review`.
- **Security** — `hydra-gates` + 8 individuele `hydra-gate-*` + vendor `trailofbits` + vendor `owasp`.
