...
tags: [Hydra, Operations, Setup, Labels, Intern, Tutorial series]
apps: []
durationMinutes: 20
...
<Outcomes title="Wat je leert in dit deel">
  ...
  <Outcome>Hoe je een spec klaar maakt en het issue triggert.</Outcome>
...
Sinds PR #134 zorgen `scripts/hydra-supervisor.sh` en `scripts/orchestrate.sh` ervoor dat ze dit bestand **zelf sourcen** bij opstart. Daarvoor moest je exporteren in je shell anders werkte het niet — een silly bug die maanden onopgemerkt bleef. Iedere nieuwe long-running entrypoint MOET dit bestand sourcen aan de start; dat is een repo-wide contract.
:::
...
In de doel-repo open je een issue (zelf, of laat Specter het doen). Op dat issue zet je:
...
1. **Wat is het probleem dat <code>HYDRA_LABEL_PREFIX</code> oplost, en wanneer hoef je hem expliciet NIET te zetten?**  
   *(Hint: denk aan twee devs op dezelfde target-repo's; en denk aan productie/CI waar één canonieke set labels juist gewenst is.)*

2. **Welke labels worden door <code>HYDRA_LABEL_PREFIX</code> WEL en welke NIET geprefixed?**  
   *(Hint: stage-labels zoals <code>build:queued</code> en herstel-labels zoals <code>retry:queued</code> wel; metadata zoals <code>yolo</code> en <code>openspec</code> niet — die zijn gedeeld.)*

3. **Waarom roep je <code>./scripts/hydra-label.sh</code> aan in plaats van <code>gh issue edit --add-label</code> bij het triggeren van een run?**  
   *(Hint: denk aan board-sync en het automatisch respecteren van de label-prefix.)*

4. **Wat gebeurt er als je <code>HYDRA_LABEL_PREFIX</code> wijzigt terwijl er nog een Hydra-run loopt? En wat is de juiste werkwijze als je toch wilt wijzigen?**  
   *(Hint: niets goeds — eerst de pijplijn drainen tot alles done/needs-input is.)*
