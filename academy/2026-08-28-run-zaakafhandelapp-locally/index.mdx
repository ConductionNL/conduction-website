---
slug: run-zaakafhandelapp-locally
title: Run Zaakafhandelapp locally
contentType: tutorial
heroIcon: folder-open
authors: [conduction]
date: 2026-08-28
summary: One compose file and about ten minutes. You end with Zaakafhandelapp running on your own machine on real Nextcloud, backed by OpenRegister, with a way to check that it actually worked rather than merely loaded.
tags: [Demo, Docker, Local, Zaakafhandelapp, OpenRegister]
apps: [zaakafhandelapp, openregister, nldesign, openconnector]
durationMinutes: 10
---
...
Zaakafhandelapp is a Nextcloud app built on OpenRegister. Reading about what it does is not the same as clicking through it, and standing up a Nextcloud by hand to find out is more work than most people will do on a first look.
...
<Outcomes>
  <Outcome>Zaakafhandelapp running locally on real Nextcloud, with OpenRegister behind it.</Outcome>
...
| App | Why |
| --- | --- |
| `openregister` | **Required.** Zaakafhandelapp declares its registers and schemas against OpenRegister. |
...
| `zaakafhandelapp` | The app this tutorial is about. |

That OpenRegister dependency is **not declared** in `appinfo/info.xml`. No app in the fleet declares an `<app>` dependency, so nothing stops the App Store installing Zaakafhandelapp on its own — it would then load, find no register to attach to, and show you an empty app rather than an error. The compose file encodes the dependency the manifest does not.
...
Zaakafhandelapp ships with more than fits in one sitting. These are the parts worth opening first:

- **ZGW case management**
- **ZGW API (ZRC, ZTC, DRC, BRC)**
- **Case lifecycle**
- **Customer contact**
- **Customer contacts in Nextcloud**
...
Nextcloud is waiting for an upgrade. Run `docker compose -f zaakafhandelapp-compose.yaml exec -u www-data nextcloud php occ upgrade`.
</TroubleshootingItem>

<TroubleshootingItem symptom="The port is already in use">
Set another one: `DEMO_PORT=9000 docker compose -f zaakafhandelapp-compose.yaml up -d`. The port also has to appear in the trusted-domain list, which the compose file handles for you from the same variable.
</TroubleshootingItem>
...
<NextSteps title="Next step" lede="A running app is the entry point, not the destination.">
  ...
  <NextStep href="/apps/zaakafhandelapp" title="What Zaakafhandelapp does" cta="Read more">
