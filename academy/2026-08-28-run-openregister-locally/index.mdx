---
slug: run-openregister-locally
title: Run OpenRegister locally
contentType: tutorial
heroIcon: database
authors: [conduction]
date: 2026-08-28
summary: One compose file and about ten minutes. You end with OpenRegister running on your own machine on real Nextcloud, backed by OpenRegister, with a way to check that it actually worked rather than merely loaded.
tags: [Demo, Docker, Local, OpenRegister, OpenRegister]
apps: [openregister, nldesign, openconnector]
durationMinutes: 10
---
...
OpenRegister is a Nextcloud app built on OpenRegister. Reading about what it does is not the same as clicking through it, and standing up a Nextcloud by hand to find out is more work than most people will do on a first look.
...
<Outcomes>
  <Outcome>OpenRegister running locally on real Nextcloud, with OpenRegister behind it.</Outcome>
...
| App | Why |
| --- | --- |
| `openregister` | **Required.** OpenRegister declares its registers and schemas against OpenRegister. |
...
| `openregister` | The app this tutorial is about. |

That OpenRegister dependency is **not declared** in `appinfo/info.xml`. No app in the fleet declares an `<app>` dependency, so nothing stops the App Store installing OpenRegister on its own — it would then load, find no register to attach to, and show you an empty app rather than an error. The compose file encodes the dependency the manifest does not.
...
OpenRegister ships with more than fits in one sitting. These are the parts worth opening first:

- **Schema-driven registers**
- **Immutable audit trail**
- **Access control to the property level**
- **Organisation multi-tenancy**
- **REST and bulk API for every object**
...
Set another one: `DEMO_PORT=9000 docker compose -f openregister-compose.yaml up -d`. The port also has to appear in the trusted-domain list, which the compose file handles for you from the same variable.
</TroubleshootingItem>
...
<NextSteps title="Next step" lede="A running app is the entry point, not the destination.">
  ...
  <NextStep href="/apps/openregister" title="What OpenRegister does" cta="Read more">
