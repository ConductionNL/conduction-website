...
Eigen `docker-compose.yml`, de officiële [`nextcloud`-image](https://hub.docker.com/_/nextcloud), Postgres en Redis ernaast. Dit is hoe wij Conduction-apps draaien voor de meeste gebruikers en hoe je [lokaal Nextcloud draait](/academy/nextcloud-lokaal-draaien). Volledige config-controle, je eigen reverse proxy ervoor (Traefik, Caddy, Nginx Proxy Manager), je eigen TLS-routine.
...
- **De meeste Conduction-gebruikers:** optie 2a, Docker Compose, met externe Postgres en S3-primary-storage, achter Traefik. Voorspelbaar, debugbaar, en je leest het terug in één compose-file.
