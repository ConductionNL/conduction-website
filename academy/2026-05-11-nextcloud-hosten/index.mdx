...
Een Nextcloud opzetten kan op drie manieren, en de "beste" hangt af van wie je bent en wat je al draait. Een homelab op een NUC vraagt iets anders dan een gemeente met tien bestuurslagen en een ISO-audit. Deze tutorial zet de drie hoofdopties naast elkaar, met voor- en nadelen en een beslismatrix om snel te kiezen. Niets vervangt de officiële [Nextcloud admin docs](https://docs.nextcloud.com/server/latest/admin_manual/), maar dit is de korte route naar de juiste hoofdstukken.
...
<Prerequisites title="Wat je nodig hebt">
  <PrerequisiteItem>Een idee van je doelgroep: alleen jezelf, een klein team, of een productiedienst voor meerdere klanten.</PrerequisiteItem>
...
**Schaalbaarheid:** Een enkel VM met 8 vCPU en 16 GB RAM trekt een paar honderd actieve users. Voor meer wil je een [scale-out-setup](https://docs.nextcloud.com/server/latest/admin_manual/installation/deployment_recommendations.html): meerdere Nextcloud-frontends achter een load balancer, met gedeelde Redis, gedeelde S3-compatibele object storage en een aparte database-host. Dat kán op bare metal, maar dan ben je effectief Kubernetes aan het uitvinden zonder Kubernetes te draaien.
...
Eigen `docker-compose.yml`, de officiële [`nextcloud`-image](https://hub.docker.com/_/nextcloud), Postgres en Redis ernaast. Dit is hoe wij Conduction-apps draaien voor de meeste klanten en hoe je [lokaal Nextcloud draait](/academy/nextcloud-lokaal-draaien). Volledige config-controle, je eigen reverse proxy ervoor (Traefik, Caddy, Nginx Proxy Manager), je eigen TLS-routine.
...
**Schaalbaarheid:** Beide opties zijn single-host. Voor échte horizontale schaal moet je richting Kubernetes of bare-metal scale-out. Voor 99% van de teams is single-host op een fatsoenlijke VM ruim voldoende — een dual-core 8 GB host trekt makkelijk dertig à vijftig actieve users.
...
**Schaalbaarheid:** Hoog. Meerdere Nextcloud-pods achter een service, gedeelde primary storage op S3, gedeelde Redis, externe gemanagede database. Dit is de enige optie die multi-region, multi-tenant tot in de honderden actieve users per minuut realistisch dekt.
...
Als je twijfelt tussen 2a en 2b: begin met **AIO**. Loop je tegen de [AIO-limitaties](https://github.com/nextcloud/all-in-one#limitations) (sub-directory, meerdere domeinen, non-standaard poort), migreer dan naar 2a — dat is een avond werk met een database-export.
...
Conduction levert apps die op de officiële Nextcloud-image draaien — onafhankelijk van hostingkeuze. Wat we in de praktijk zien:

- **De meeste Conduction-klanten:** optie 2a, Docker Compose, met externe Postgres en S3-primary-storage, achter Traefik. Voorspelbaar, debugbaar, en je leest het terug in één compose-file.
- **Homelabs en eerste demo's:** optie 2b (AIO) of [`docker-compose up`](/academy/nextcloud-lokaal-draaien). Tien minuten van niets naar werkende cloud.
- **Grote multi-tenant deployments:** optie 3, Helm, met cert-manager en gemanagede database. Zeldzaam, maar wel groeiend.

Geen van die keuzes is "de juiste" in absolute zin. De beslismatrix is de juiste, voor jou.
...
<ContactCta
  ...
  body="Hosten voor een gemeente, een onderwijsinstelling, of een SaaS-product? Mail ons — we hebben de drie smaken alle drie in productie zien gaan."
