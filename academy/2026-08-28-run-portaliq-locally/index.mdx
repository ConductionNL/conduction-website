---
slug: run-portaliq-locally
title: Run Portaliq locally
contentType: tutorial
heroIcon: globe
authors: [conduction]
date: 2026-08-28
summary: One compose file and about ten minutes. You end with Portaliq running on your own machine on real Nextcloud, backed by OpenRegister, with a way to check that it actually worked rather than merely loaded.
tags: [Demo, Docker, Local, Portaliq, OpenRegister]
apps: [portaliq, openregister, nldesign, openconnector]
durationMinutes: 10
---
...
Portaliq is a Nextcloud app built on OpenRegister. Reading about what it does is not the same as clicking through it, and standing up a Nextcloud by hand to find out is more work than most people will do on a first look.
...
<Outcomes>
  <Outcome>Portaliq running locally on real Nextcloud, with OpenRegister behind it.</Outcome>
...
| App | Why |
| --- | --- |
| `openregister` | **Required.** Portaliq declares its registers and schemas against OpenRegister. |
...
| `portaliq` | The app this tutorial is about. |

That OpenRegister dependency is **not declared** in `appinfo/info.xml`. No app in the fleet declares an `<app>` dependency, so nothing stops the App Store installing Portaliq on its own — it would then load, find no register to attach to, and show you an empty app rather than an error. The compose file encodes the dependency the manifest does not.
...
Portaliq ships with more than fits in one sitting. These are the parts worth opening first:

- **Public portals**
- **Themed sites over register data**
- **Portal domains and verification**
...
Nextcloud is waiting for an upgrade. Run `docker compose -f portaliq-compose.yaml exec -u www-data nextcloud php occ upgrade`.
</TroubleshootingItem>

<TroubleshootingItem symptom="The port is already in use">
Set another one: `DEMO_PORT=9000 docker compose -f portaliq-compose.yaml up -d`. The port also has to appear in the trusted-domain list, which the compose file handles for you from the same variable.
</TroubleshootingItem>
...
<NextSteps title="Next step" lede="A running app is the entry point, not the destination.">
  ...
  <NextStep href="/apps/portaliq" title="What Portaliq does" cta="Read more">
