...
| Veld | Functie |
|---|---|
| `name` | Moet **exact gelijk** zijn aan de mapnaam. Folder heet `review-pr` → `name: review-pr`. Wijkt het af, dan triggert het slash-command niet. |
...
De mapnaam, het `name`-veld én het slash-commando moeten allemaal exact gelijk zijn.

## Skill vs. prompt vs. command vs. agent

Een veelgehoorde vraag: wanneer is een skill de juiste keuze, en wanneer iets anders? Vier categorieën, kort:
...
<HexCard title="Slash-command (legacy)">
  In oudere versies van Claude Code stonden er files in <code>.claude/commands/*.md</code> als aparte categorie. Anthropic heeft die samengevoegd: een file in <code>.claude/commands/</code> werkt nog steeds, maar skills zijn nu het aanbevolen format en bieden meer features (supporting files, frontmatter-controle, auto-activering). Nieuw werk: altijd een skill.
</HexCard>
...
**1. Wat is het verschil tussen een skill en een command in Claude Code?**

<Details summary="Hint">

Denk aan de geschiedenis: in oudere versies waren het twee aparte mechanismen. Wat is er sindsdien gebeurd?

</Details>

<Details summary="Antwoord">

In oudere versies van Claude Code waren `.claude/commands/*.md` files een aparte categorie naast skills. Anthropic heeft die samengevoegd: een file in `.claude/commands/` werkt nog steeds, maar **skills zijn nu het aanbevolen format**.

Skills hebben extra mogelijkheden die commands niet hebben:

- Supporting files in subfolders (`templates/`, `references/`, `examples/`, `scripts/`)
- Frontmatter-controle over hoe ze geactiveerd worden (handmatig, automatisch, of beide)
- Auto-activering door Claude zelf op basis van de `description`

Praktisch: als iemand het heeft over "de `/review-pr` slash-command", dan bedoelt diegene de skill die in `.claude/skills/review-pr/` staat. De naam achter de schuine streep is altijd de naam van een skill-map. Voor nieuw werk: schrijf een skill, geen command.
...
- **`name`** — moet exact gelijk zijn aan de mapnaam (folder `review-pr` → `name: review-pr`). Wijkt het af, dan triggert het slash-command niet.
