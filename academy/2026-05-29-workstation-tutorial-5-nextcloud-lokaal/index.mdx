...
> **Alternative compose with profiles — optional, extra.** The Conduction [`openregister`](https://github.com/ConductionNL/openregister) repo also ships its own `docker-compose.yml` with `--profile` flags (`ai`, `ui`, `exapps`, `solr`, `ollama`, `openproject`, `xwiki`, `openzaak`, `openklant`, and more) — documented in [`docs/claude/docker.md`](https://github.com/ConductionNL/.github/blob/main/docs/claude/docker.md). Useful when you need those heavier services, but not required for the workstation chain and not what most Conduction devs run day-to-day. Stick with `nextcloud-docker-dev` for now.
