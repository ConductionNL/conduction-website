...
**Beginner-friendly demo — [Run Nextcloud locally with Docker](/academy/nextcloud-lokaal-draaien).** A 30-minute walkthrough that pastes a standalone `docker-compose.yml` into a fresh folder. Aimed at someone who has never used Docker or a terminal. Good for a first run-through; not the layout you'll use day-to-day for actual app development (the standalone compose doesn't have an `apps-extra/` mount, so you can't iterate on Conduction app code that way).
